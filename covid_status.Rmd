---
title: "Philadelphia COVID Guidance for Schools"
...
The purpose of this website is to apply COVID-19 school recommendations from trusted sources to the current situation in Philadelphia. To this end, recommended measures and thresholds from the following publications are calcuated, and published recommendations based on these indicators are presented.
...
url_hospitilizations <- URLencode("https://phl.carto.com/api/v2/sql?q=SELECT * FROM covid_hospitalizations_by_date")

data_hospitilizations_import <- fromJSON(url_hospitilizations)

hospitilization_data <- data_hospitilizations_import$rows %>% 
  select(-etl_timestamp, -the_geom_webmercator, -the_geom, -cartodb_id) %>% 
  mutate(date = as.Date(date)) %>% 
  filter(hospitalized == "Yes") %>% 
  drop_na(date) %>% 
  complete(date = full_seq(date, period = 1), fill = list(hospitalized = "Yes", positive = NA))

hospitilization_data$count_imputed <- na.approx(hospitilization_data$count, maxgap = 7) 

hospitilization_data <- hospitilization_data %>% 
  
  mutate(count_rate = count_imputed/1584064*100000,
         ut_austin_status = case_when(count_rate < 1.037 ~ "Stage 1",
                                      count_rate >= 1.037 & count_rate < 4.045 ~ "Stage 2",
                                      count_rate >= 4.045 & count_rate < 7.259 ~ "Stage 3",
                                      count_rate >= 7.259 & count_rate < 12.756 ~ "Stage 4", 
                                      count_rate > 12.756 ~ "Stage 5")
  )
