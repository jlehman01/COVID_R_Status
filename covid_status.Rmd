...
unzip(zipfile = "covid_archive_github.zip")
...
hospitilizations_under_20_complete <- cases_by_age %>% 
  filter(age == "<20") %>% 
  mutate(date = as.Date(etl_timestamp)) %>% 
...
hospitilizations_under_20_complete <- hospitilizations_under_20_complete %>% 
  mutate(new_hospitilizations = Yes_imputed - lag(Yes_imputed, n=1),
...
covid_hospitilizations_under_20 <- ggplot(data = hospitilizations_under_20_complete, aes(x=date,
  ...
  geom_col(fill="pink")+
  geom_line(aes(y = new_hospitilizations_7_mean), 
            color = "red", 
            size = .75)+
  labs(title="New Philadelphia COVID Hospitalizations for < 20 Population",
       y="New Hospitalizations / Day") +
      theme_bw()
